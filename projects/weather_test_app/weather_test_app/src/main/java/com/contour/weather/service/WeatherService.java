...
public class WeatherService {
    ...
    /**
     * Weather update records from db by given location
     */
    public List<WeatherUpdate> getWeatherUpdateByLocation( String country
        ...
        final List<WeatherUpdate> weatherUpdates = new ArrayList<>();
        Iterable<WeatherUpdate> iterable =  weatherUpdateRepo.fetchByLocation(city);
        for (WeatherUpdate w : iterable) {
            weatherUpdates.add(w);
        }
        return weatherUpdates;
    ...
    /**
     * Returns most recent entry from the db for the given location
     */
    public WeatherUpdate getCurrentWeatherUpdateByLocation( String country
            , String state
            , String city){
        logger.debug("Fetching Weather updates by location"+country+", " +state+" , "+city);
        validateInput(country, state, city);
        final List<WeatherUpdate> weatherUpdates = new ArrayList<>();
        Iterable<WeatherUpdate> iterable = weatherUpdateRepo.findTopByCityOrderByWeatherTimeDesc(city);
        for (WeatherUpdate w : iterable) {
            weatherUpdates.add(w);
        }
        if(weatherUpdates.size() > 0) { return weatherUpdates.get(0);}
        else {return new WeatherUpdate();}
    ...
    /**
     * cleanup old data record by calling stored procedure till the given time
